"""MPCC path following with moving obstacles and uncertainty-aware risk metrics.

The most advanced example: dynamic obstacles with covariance propagation and
a mean-variance risk metric that accounts for prediction uncertainty when
evaluating collision costs.
"""
...
class Result:
    """Outcome of a planning simulation with uncertain obstacles."""
...
def create():
    ...
    obstacle_simulator = create_obstacles.dynamic(
        positions=array(
            [
                [25.0, 22.5],
                [55.0, 0.0],
    ...
    risk_metric = risk.mean_variance(gamma=0.5, sample_count=10)
...
SEED = "doc-dynamic-obstacles-uncertain"
MAX_CONTOURING_ERROR = 5.0
MAX_LAG_ERROR = 12.0
