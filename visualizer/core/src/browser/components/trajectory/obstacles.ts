...
const boxObstaclesUpdater: TraceUpdateCreator = (data, index) => {
    ...
    if (wheelbase == null || vehicleWidth == null) {
        throw new Error(
            "The vehicle's wheelbase and width must be provided, when rendering obstacles as boxes.",
        );
    }
...
const circleObstaclesUpdater: TraceUpdateCreator = (data, index) => {
    if (data.info.obstacleRadius == null) {
        throw new Error(
            "The obstacle radius must be provided, when rendering obstacles as circles.",
        );
    }
