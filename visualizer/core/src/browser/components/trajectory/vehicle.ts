...
const boxVehicleUpdater: TraceUpdateCreator = (data, index) => {
    ...
    if (wheelbase == null || vehicleWidth == null) {
        throw new Error(
            "The vehicle's wheelbase and width must be provided, when rendering the ego vehicle as a box.",
        );
    }
...
const circleVehicleUpdater: TraceUpdateCreator = (data, index) => {
    if (data.info.vehicleRadius == null) {
        throw new Error(
            "The vehicle's radius must be provided, when rendering the ego vehicle as a circle.",
        );
    }
